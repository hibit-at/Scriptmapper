...
スクリプトをある範囲で一括コピーします。マップの展開が同じ部分で、以前書いたブックマークを使い回す時に便利です。例えば、100グリッド目で`copy40`と書いたとします。その次のブックマークが120だった場合、コピーを**貼り付ける**範囲は100~120になります。この長さは20です。そしてコピーの**元となる**範囲は40から始まり、同じ長さを持つ部分になります。つまり、40~60の部分が100~120の範囲になります。この時、終点となる60や120は**含まれない**ことに注意してください。

ex) `copy40`

※なぜ終点が含まれないのか、また、終了地点を明示しなくてよいのかについて説明します。上の例だとグリッド120の位置には別のスクリプトがすでに存在しているはずで、そこにコピーするのは避けなければいけません。では、それより手前（例えば119）に終了地点を作れば、そのようなブックマークにあまり意味はありません。コピー対象はある程度まとまった範囲（例えば8小節）を形成しているはずなので、そのような範囲同士の位置関係を考える場合は、終了地点は「次の範囲の開始地点**の直前まで**」と決めた方が、処理が合理的です。
