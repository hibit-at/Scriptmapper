...
- コマンドが 2 つある場合（例：カメラの`def,diag3`）は、1 つ目のコマンドから 2 つ目のコマンドへ連続的に変化します。

# プリセットコマンド　基本編
スクリプトの先頭に以下の文字が書かれていた場合、プリセットコマンドとして認識されます。

※以下、位置に関わるパラメータは**メートル基準**、角度に関わるパラメータは**0～360度基準**になります（not ラジアン）。

|名前|説明|パラメータ|例|
|---|---|---|---|
|`def`|真後ろから見下ろす視点です|-|`def`|
|`random`|ランダムな座標にカメラを置きます。|半径|`random3`|
|`front`|アバターの真正面にカメラを置きます。高さは1.5m（ほぼ顔と同じ位置）。|前方向位置|`front1`|
|`side`|アバターの真横にカメラを置きます。高さは1.5m（ほぼ顔と同じ位置）。|右方向位置|`side3`|
|`diag`|アバターの斜め後ろにカメラを置きます。高さは3m（見下ろす位置）。|右後方向位置|`diag3`|
...
|列名|機能|
|---|---|
|`label`|コマンドとして認識される名前。これと同じ名前をスクリプトに記入する。|
|`px`|カメラの x 座標|
|`py`|カメラの y 座標|
|`pz`|カメラの z 座標|
...
※パラメータの指定はマイナス`-40`、小数点開始`.5`、分数指定`5/4`でも可能です。
